...
//   const [bodyData,setBodyData]=useState([]);
    ...
    try {
      ...
      if (selectedCage === "All") {
        ...
        for (let device of listDevices) {
          imageUrl = `${process.env.REACT_APP_FETCH_LINK}images/${device}/${formattedDate}/`;
        ...
        imageUrl = `${process.env.REACT_APP_FETCH_LINK}images/${jetsonName}/${formattedDate}/`;
        const response = await fetch(imageUrl);
        const data = await response.json();
        const filteredImages = data.images.filter(
          (image) => !image.endsWith("/")
  ...
  const fetchData = async () => { //changed from useCallBAck to plain function and removed dependenc from below useEffect to remove infinite rerenders
    try {
      const dataWithBase64 = filteredImageURLs
        ...
        .map(async (imgURL) => {
          ...
          const base64Image = await loadImageAsBase64(
            `https://cvsterlite.s3.ap-south-1.amazonaws.com/${imgURL}`
          );
          ...
          const imageData = {
            ...
            time: timestamp,
            cage: cageName,
            ImageUrl: `https://cvsterlite.s3.ap-south-1.amazonaws.com/${imgURL}`,
    ...
    //   const transformedDataArr = await Promise.all(dataWithBase64);
      setTransformedData(await Promise.all(dataWithBase64));
    } catch (error) {
  ...
  },[setBaseImgArr, setBodyData, transformedData])
  ...
  useEffect(()=>{
    fetchData();
      ...
      useEffect(()=>{
        if (transformedData &&  transformedData.length===filteredImageURLs.length && bodyData.length>0 && filteredImageURLs.length>0) {
      ...
      },[bodyData.length, filteredImageURLs.length, selectedDate, setIsDataFilled, transformedData])
      useEffect(()=>{
          setBodyData([])
      },[selectedDate, setBodyData])
                ...
                .map((imgURL, index) => {
                  ...
                  return (
                    <tr key={index}>
                      ...
                      <td datalabel="IMAGE">
                        <img
                          src={`https://cvsterlite.s3.ap-south-1.amazonaws.com/${imgURL}`}
                          alt="No_Image_Available" loading="lazy"
                        />
                      </td>
                    </tr>
                  );
