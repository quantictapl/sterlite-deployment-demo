...
export default function Table({
  ...
  const [isPdfGenerateable, setISPdfGenerateable] = useState(false);
...
//   console.log(filteredImageURLs);
  ...
  const compareDates = () => {
     console.log(globalTransformedData.length)
    if (globalTransformedData.length > 0 && filteredImageURLs.length>0) {
        console.log("Compare Dates worked");
        const transformedDataDate = globalTransformedData[0].date;
        ...
        console.log("Compare Dates not worked");
      }
  };

//   useEffect(() => {
//     compareDates();
//   }, [selectedDate, globalTransformedData]);
  ...
  const fetchData =useCallback( async () => {
    try {
      ...
      globalTransformedData = transformedData;
    ...
    //     console.log(selectedDateWithoutTime)
    //     setIsDataReady(
    //       transformedDate.getTime() === selectedDateWithoutTime.getTime() && bodyData.length>0
    //     );
    //     setISPdfGenerateable( transformedDate.getTime() === selectedDateWithoutTime.getTime() && bodyData.length>0)
        
    //   } else {
    //     setIsDataReady(false);
    //     console.log("Compare Dates not worked");
    //     setISPdfGenerateable(false)
    //   }
  ...
  useEffect(()=>{
    fetchData();
  },[fetchData,filteredImageURLs,selectedDate])
  ...
  return (
    <div className="table-container">
      <button id="pdf-generate-btn" onClick={printPdf} disabled={!isPdfGenerateable}>
        Convert to PDF
      </button>
